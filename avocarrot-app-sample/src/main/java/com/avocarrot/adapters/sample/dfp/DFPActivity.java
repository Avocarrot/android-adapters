...
public class DFPActivity extends AppCompatActivity implements AdapterView.OnItemClickListener {

    @NonNull
    public static Intent buildIntent(@NonNull final Context context) {
        return new Intent(context, DFPActivity.class);
    }
    ...
    public void onItemClick(@NonNull final AdapterView<?> parent, @NonNull final View view, final int position, final long id) {
        switch (position) {
            case 0:
                IntentStarter.startDFPConfigActivity(this, IntentStarter.SCREEN_BANNER);
                break;
            case 1:
                IntentStarter.startDFPConfigActivity(this, IntentStarter.SCREEN_INTERSTITIAL);
                break;
            default:
    ...
    private static final class MainAdapter extends ArrayAdapter<String> {
        ...
        private MainAdapter(@NonNull final Context context) {
            super(context, android.R.layout.simple_list_item_1, android.R.id.text1, context.getResources().getStringArray(R.array.dfp_ad_types));
        }
    }
}
